...
class ColorMatcher @Inject constructor(
    ...
    fun getCorrectiveRecommendations(analysis: LipColorAnalysis): List<PigmentRecommendation> {
        ...
        val source: Sequence<PigmentRecommendation> = when (analysis.category) {
            LipColorCategory.PALE, LipColorCategory.PALE_PINK -> {
                allPigments.asSequence()
                    .filter { it.intensity == "light" || it.intensity == "medium" }
                    .map { pigment ->
            ...
            LipColorCategory.PIGMENTED, LipColorCategory.DARK_RED, LipColorCategory.BROWN -> {
                allPigments.asSequence().map { pigment ->
        ...
        return source.sortedByDescending { it.matchScore }.take(10).toList()
    }

    fun getDesiredResultRecommendations(targetColorHex: String): List<PigmentRecommendation> {
        return pigmentRepository.getAllPigments().asSequence()
            .map { pigment ->
                PigmentRecommendation(
                    pigment = pigment,
    ...
    fun blendPigmentResult(naturalHex: String, pigmentHex: String, intensity: Float = 0.6f): String =
        blendPigmentResultStatic(naturalHex, pigmentHex, intensity)
    ...
    companion object {
        fun blendPigmentResultStatic(naturalHex: String, pigmentHex: String, intensity: Float = 0.6f): String {
            return try {
                ...
                val natLab = rgbToLabShared(Color.red(natColor), Color.green(natColor), Color.blue(natColor))
                val pigLab = rgbToLabShared(Color.red(pigColor), Color.green(pigColor), Color.blue(pigColor))
                ...
                val rgb = labToRgbShared(blL, blA, blB)
        ...
        internal fun rgbToLabShared(r: Int, g: Int, b: Int): FloatArray {
            var rr = r / 255.0
            var gg = g / 255.0
            var bb = b / 255.0
        ...
        internal fun labToRgbShared(l: Float, a: Float, b: Float): IntArray {
